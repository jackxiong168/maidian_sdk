...
public class AppClickTrackImpl {
     ...
     */
    public static void trackExpandableListViewOnGroupClick(SensorsDataAPI sensorsDataAPI,
        ...
        try {
            ...
            ViewContext viewContext = getAppClickViewContext(expandableListView,
            ...
            JSONUtils.mergeJSONObject(getExpandListViewExtendProperty(expandableListView,
                    groupPosition, -1), properties);
            // 获取 View 自定义属性
     ...
     */
    public static void trackExpandableListViewOnChildClick(SensorsDataAPI sensorsDataAPI,
                                                           ExpandableListView expandableListView,
                                                           View view,
        ...
        try {
            ...
            // assemble interface extend property
            JSONUtils.mergeJSONObject(getExpandListViewExtendProperty(expandableListView,
                    groupPosition, childPosition), properties);
            // 获取 View 自定义属性
            JSONUtils.mergeJSONObject((JSONObject) view.getTag(com.sensorsdata.analytics.android.sdk.R.id.sensors_analytics_tag_view_properties), properties);
            trackAutoEvent(sensorsDataAPI, properties, view);
        } catch (Exception e) {
    ...
    public static void trackTabHost(SensorsDataAPI sensorsDataAPI, final String tabName) {
        try {
            ...
            ViewContext viewContext =
                    getAppClickViewContext(AutoTrackViewUtils.getTabView(tabName), TabHost.class);
            if (viewContext == null) {
                return;
            }
     ...
     * @param tab            Tab
     */
    public static void trackTabLayoutSelected(SensorsDataAPI sensorsDataAPI, Object objectTab,
                                              Object tab) {
        try {
            ...
            if (currentTabClass != null) {
                ...
                if (tabCustomView != null) {
                    if (tabCustomView.getId() != View.NO_ID && viewContext.activity != null) {
                        String resourceId =
                                viewContext.activity.getResources().getResourceEntryName(tabCustomView.getId());
                    ...
                    JSONObject p =
                            (JSONObject) tabCustomView.getTag(com.sensorsdata.analytics.android.sdk.R.id.sensors_analytics_tag_view_properties);
     ...
     */
    public static void trackMenuItem(SensorsDataAPI sensorsDataAPI, final Object object,
        ...
        try {
            ...
            ViewContext viewContext = getAppClickViewContext(object,
                    WindowHelper.getClickView(menuItem), MenuItem.class);
            if (viewContext == null) {
                return;
            }
            ...
            try {
                ...
                if (viewContext.activity != null) {
                    String idString =
                            viewContext.activity.getResources().getResourceEntryName(menuItem.getItemId());
                    if (!TextUtils.isEmpty(idString)) {
                        properties.put(AopConstants.ELEMENT_ID, idString);
                    }
                }
     ...
     */
    public static void trackRadioGroup(SensorsDataAPI sensorsDataAPI, RadioGroup view,
                                       int checkedId) {
        try {
            ...
            try {
                if (viewContext.activity != null) {
                    ...
                    if (radioButton != null && !TextUtils.isEmpty(radioButton.getText())) {
                        properties.put(AopConstants.ELEMENT_CONTENT,
            ...
            //获取 View 自定义属性
            JSONObject p =
                    (JSONObject) view.getTag(com.sensorsdata.analytics.android.sdk.R.id.sensors_analytics_tag_view_properties);
            JSONUtils.mergeJSONObject(p, properties);
     ...
     */
    public static void trackDialog(SensorsDataAPI sensorsDataAPI, DialogInterface dialogInterface
            , int whichButton) {
        try {
            ...
            try {
                ...
                if (window != null && window.isActive()) {
                    String idString =
                            (String) dialog.getWindow().getDecorView().getTag(com.sensorsdata.analytics.android.sdk.R.id.sensors_analytics_tag_view_id);
                    if (!TextUtils.isEmpty(idString)) {
                        properties.put(AopConstants.ELEMENT_ID, idString);
                    }
                }
     ...
     */
    public static void trackListView(SensorsDataAPI sensorsDataAPI, AdapterView<?> adapterView,
        ...
        try {
            ...
            if (adapter instanceof SensorsAdapterViewItemTrackProperties) {
                try {
                    SensorsAdapterViewItemTrackProperties objectProperties =
                            (SensorsAdapterViewItemTrackProperties) adapter;
                    JSONObject jsonObject =
                            objectProperties.getSensorsItemTrackProperties(position);
     ...
     */
    public static void trackViewOnClick(SensorsDataAPI sensorsDataAPI, View view,
                                        boolean isFromUser) {
        try {
     ...
     */
    public static void track(final SensorsDataAPI sensorsDataAPI, final String eventName,
                             String properties) {
        try {
    ...
    private static void trackAutoEvent(final SensorsDataAPI sensorsDataAPI,
                                       final JSONObject properties, final View view) {
        ...
        SAEventManager.getInstance().trackQueueEvent(new Runnable() {
            ...
            public void run() {
                ...
                SALog.i(TAG, "过滤的点击事件properties: \n" + properties.toString());
                // 过滤点击事件，获取properties的$screen_name和$element_selector，如果不在配置文件（数据库）中就丢弃,
                // 有就取出对应event_id设置给InputData
                String eventId = "";
                String elementPosition = "";
                try {
                    String screenName = properties.getString(AopConstants.SCREEN_NAME);
                    String elementSelector = properties.getString(AopConstants.ELEMENT_SELECTOR);
                    //处理列表item点击
                    String elementPath = properties.getString(AopConstants.ELEMENT_PATH);
                    if (!TextUtils.isEmpty(elementPath) && elementPath.endsWith("[-]")) {
                        elementSelector = elementSelector.substring(0, elementSelector.length() - 3);
                        elementPosition = properties.getString(AopConstants.ELEMENT_POSITION);
                    }
                    eventId = DbAdapter.getInstance().queryVisualConfigEventId(screenName,
                            elementSelector, DbParams.VISUAL_CONFIG_TYPE_CLICK, elementPosition);
                    SALog.i(TAG,
                            "过滤出来的点击事件的event_id: " + eventId + "=====控件路径elementSelector:" + elementSelector);
                } catch (Exception e) {
                    SALog.printStackTrace(e);
                }
                if (!TextUtils.isEmpty(eventId)) {
                    sensorsDataAPI.getSAContextManager().
                            trackEvent(new InputData()
                                    .setEventName(AopConstants.APP_CLICK_EVENT_NAME)
    ...
    private static ViewContext getAppClickViewContext(Object parent, View view,
                                                      Class<?> classType) {
    ...
    private static JSONObject getExpandListViewExtendProperty(ExpandableListView expandableListView, int groupPosition, int childPosition) {
        try {
            ...
            if (listAdapter instanceof SensorsExpandableListViewItemTrackProperties) {
                SensorsExpandableListViewItemTrackProperties trackProperties =
                        (SensorsExpandableListViewItemTrackProperties) listAdapter;
                return childPosition == -1 ?
                        trackProperties.getSensorsGroupItemTrackProperties(groupPosition)
                        : trackProperties.getSensorsChildItemTrackProperties(groupPosition,
                        childPosition);
            }
        } catch (Exception e) {
            SALog.printStackTrace(e);
    ...
    public static JSONObject buildPageProperty(Activity activity, Object fragment) {
        ...
        // fragmentName
        if (fragment != null) {
            JSONUtils.mergeJSONObject(SAPageTools.getFragmentPageInfo(activity, fragment),
                    properties);
