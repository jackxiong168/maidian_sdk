...
public class AutoTrackProtocolIml implements AutoTrackProtocol {
    ...
    public boolean isAutoTrackEventTypeIgnored(int autoTrackEventType) {
        if (mContextManager.getRemoteManager() != null) {
            Boolean isIgnored =
                    mContextManager.getRemoteManager().isAutoTrackEventTypeIgnored(autoTrackEventType);
            if (isIgnored != null) {
                if (isIgnored) {
                    SALog.i(TAG,
                            "remote config: " + SensorsDataAPI.AutoTrackEventType.autoTrackEventName(autoTrackEventType) + " is ignored by remote config");
                }
                return isIgnored;
            }
        }
    ...
    public void setViewID(View view, String viewID) {
        if (view != null && !TextUtils.isEmpty(viewID)) {
            view.setTag(com.sensorsdata.analytics.android.sdk.R.id.sensors_analytics_tag_view_id,
                    viewID);
        }
    }
                        ...
                        if (!TextUtils.isEmpty(url) || cloneProperties != null) {
                            ...
                            if (cloneProperties != null) {
                                if (cloneProperties.has("$title")) {
                                    SAPageTools.setCurrentScreenTitle(cloneProperties.getString(
                                            "$title"));
                                } else {
                            ...
                            SALog.i(TAG, "过滤的页面浏览事件properties: \n" + trackProperties.toString());
                            // 过滤页面浏览事件
                            String screenName = trackProperties.getString(AopConstants.SCREEN_NAME);
                            String eventId =
                                    DbAdapter.getInstance().queryVisualConfigEventId(screenName,
                                            null, DbParams.VISUAL_CONFIG_TYPE_BROWSE,null);
                            if (!TextUtils.isEmpty(eventId)) {
                                SAEventManager.getInstance().trackEvent(new InputData()
                                        .setEventName("$AppViewScreen")
        ...
        try {
            ...
            if (fragment.getClass().isAnnotationPresent(SensorsDataFragmentTitle.class)) {
                SensorsDataFragmentTitle sensorsDataFragmentTitle =
                        fragment.getClass().getAnnotation(SensorsDataFragmentTitle.class);
            ...
            if (Build.VERSION.SDK_INT >= 11) {
                ...
                if (activity != null) {
                    ...
                    screenName = String.format(TimeUtils.SDK_LOCALE, "%s|%s",
                            activity.getClass().getCanonicalName(), screenName);
                }
            }
    ...
    public void trackViewAppClick(final View view, final JSONObject properties) {
        ...
        try {
            JSONObject cloneProperties = properties != null ?
                    JSONUtils.cloneJsonObject(properties) : new JSONObject();
            final JSONObject propertyJson = AopUtil.injectClickInfo(new ViewContext(view),
                        ...
                        try {
                            ...
                            try {
                                ...
                                String elementSelector =
                                        properties.getString(AopConstants.ELEMENT_SELECTOR);
                                //处理列表item点击
