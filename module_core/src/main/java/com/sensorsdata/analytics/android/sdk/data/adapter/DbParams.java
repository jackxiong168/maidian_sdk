...
public class DbParams {
    ...
    //可视化后台圈选的点击事件配置表
    public static final String TABLE_VISUAL_CONFIGS = "visual_configs";
    ...
    private final Uri mUri, mActivityStartCountUri, mAppStartTimeUri,
            ...
            mEnableSDKUri, mDisableSDKUri, mRemoteConfigUri, mUserIdentities, mLoginIdKeyUri,
            mPushIdUri, mVisualConfigs;
    /* 替换 APP_END_DATA 数据，使用新的 SP 文件保存 */
    public static final String APP_EXIT_DATA = "app_exit_data";
    //可视化配置表字段
    public static final String KEY_SCREEN_NAME = "screen_name";
    public static final String KEY_ELEMENT_SELECTOR = "element_selector";
    public static final String KEY_EVENT_ID = "event_id";
    public static final String KEY_EVENT_TYPE = "event_type";
    public static final String VISUAL_CONFIG_TYPE_CLICK="click";
    public static final String VISUAL_CONFIG_TYPE_BROWSE="browse";
    //列表item索引
    ...
    public interface PersistentName {
        ...
        String VISUAL_CONFIGS_SIGN="visualConfigsMd5";
    }

    private DbParams(String packageName) {
        mUri = Uri.parse("content://" + packageName + ".SensorsDataContentProvider/" + TABLE_EVENTS);
        mActivityStartCountUri = Uri.parse("content://" + packageName +
                ".SensorsDataContentProvider/" + TABLE_ACTIVITY_START_COUNT);
        mAppStartTimeUri =
                Uri.parse("content://" + packageName + ".SensorsDataContentProvider/" + TABLE_APP_START_TIME);
        mAppExitDataUri =
                Uri.parse("content://" + packageName + ".SensorsDataContentProvider/" + APP_EXIT_DATA);
        mSessionTimeUri =
                Uri.parse("content://" + packageName + ".SensorsDataContentProvider/" + TABLE_SESSION_INTERVAL_TIME);
        mLoginIdUri =
                Uri.parse("content://" + packageName + ".SensorsDataContentProvider/" + PersistentName.LOGIN_ID);
        mLoginIdKeyUri =
                Uri.parse("content://" + packageName + ".SensorsDataContentProvider/" + PersistentName.PERSISTENT_LOGIN_ID_KEY);
        mChannelPersistentUri = Uri.parse("content://" + packageName +
                ".SensorsDataContentProvider/" + TABLE_CHANNEL_PERSISTENT);
        mSubProcessUri =
                Uri.parse("content://" + packageName + ".SensorsDataContentProvider/" + PersistentName.SUB_PROCESS_FLUSH_DATA);
        mEnableSDKUri =
                Uri.parse("content://" + packageName + ".SensorsDataContentProvider/" + TABLE_DATA_ENABLE_SDK);
        mDisableSDKUri =
                Uri.parse("content://" + packageName + ".SensorsDataContentProvider/" + TABLE_DATA_DISABLE_SDK);
        mRemoteConfigUri =
                Uri.parse("content://" + packageName + ".SensorsDataContentProvider/" + PersistentName.REMOTE_CONFIG);
        mUserIdentities =
                Uri.parse("content://" + packageName + ".SensorsDataContentProvider/" + PersistentName.PERSISTENT_USER_ID);
        mPushIdUri =
                Uri.parse("content://" + packageName + ".SensorsDataContentProvider/" + PUSH_ID_KEY);
        mVisualConfigs =
                Uri.parse("content://" + packageName + ".SensorsDataContentProvider/" + TABLE_VISUAL_CONFIGS);
    }
    ...
    public static DbParams getInstance() {
        if (instance == null) {
            throw new IllegalStateException("The static method getInstance(String packageName) " +
     ...
     * @return
     */
   public Uri getVisualConfigsUri() {
        return mVisualConfigs;
    }
