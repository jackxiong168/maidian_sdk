...
public class DbAdapter {
    ...
    private VisualConfigsDataOperation mVisualConfigsOperation;
    ...
    private DbAdapter(SAContextManager saContextManager) {
        ...
        if (saContextManager.getInternalConfigs().saConfigOptions.isEnableEncrypt()) {
            mTrackEventOperation =
                    new EncryptDataOperation(saContextManager.getContext().getApplicationContext());
        } else {
            mTrackEventOperation =
                    new EventDataOperation(saContextManager.getContext().getApplicationContext());
        }
        mPersistentOperation =
                new PersistentDataOperation(saContextManager.getContext().getApplicationContext());
        mVisualConfigsOperation =
                new VisualConfigsDataOperation(saContextManager.getContext().getApplicationContext());
    }
    ...
    public static DbAdapter getInstance() {
        if (instance == null) {
            throw new IllegalStateException("The static method getInstance(SAContextManager " +
                    "saContextManager) should be called before calling getInstance()");
        }
        return instance;
    }
     ...
     * @param contentValues
     */
    public void addVisualConfigs(ContentValues contentValues) {
        if (contentValues != null && contentValues.size() > 0) {
            mVisualConfigsOperation.insertData(DbParams.getInstance().getVisualConfigsUri(),
                    contentValues);
        }
    }
     ...
     * @param operations
     */
    public void addVisualConfigsBatch(ArrayList<ContentProviderOperation> operations) {
        mVisualConfigsOperation.insertInBatch(operations);
    }
     ...
     * 删除可视化后台配置表
     */
    public void deleteAllVisualConfigs() {
        mVisualConfigsOperation.deleteData(mDbParams.getVisualConfigsUri(), DbParams.DB_DELETE_ALL);
    }
    ...
    public void commitActivityCount(int activityCount) {
        try {
            mPersistentOperation.insertData(mDbParams.getActivityStartCountUri(),
                    new JSONObject().put(DbParams.VALUE, activityCount));
    ...
    public void commitAppStartTime(long appStartTime) {
        try {
            mPersistentOperation.insertData(mDbParams.getAppStartTimeUri(),
                    new JSONObject().put(DbParams.VALUE, appStartTime));
        } catch (JSONException e) {
            SALog.printStackTrace(e);
        }
    }
    ...
    public void commitAppExitData(String appEndData) {
        try {
            mPersistentOperation.insertData(mDbParams.getAppExitDataUri(),
                    new JSONObject().put(DbParams.VALUE, appEndData));
        } catch (JSONException e) {
            SALog.printStackTrace(e);
        }
    }
    ...
    public void commitLoginId(String loginId) {
        try {
            mPersistentOperation.insertData(mDbParams.getLoginIdUri(),
                    new JSONObject().put(DbParams.VALUE, loginId));
        } catch (JSONException e) {
            SALog.printStackTrace(e);
        }
    }
    ...
    public void commitSessionIntervalTime(int sessionIntervalTime) {
        try {
            mPersistentOperation.insertData(mDbParams.getSessionTimeUri(),
                    new JSONObject().put(DbParams.VALUE, sessionIntervalTime));
        } catch (JSONException e) {
            SALog.printStackTrace(e);
        }
    }
    ...
    public boolean isFirstChannelEvent(String[] eventName) {
        try {
            return mTrackEventOperation.queryDataCount(mDbParams.getChannelPersistentUri(),
                    null,
                    DbParams.KEY_CHANNEL_EVENT_NAME + " = ? or " + DbParams.KEY_CHANNEL_EVENT_NAME + " = ? ", eventName, null) <= 0;
        } catch (Exception e) {
            SALog.printStackTrace(e);
        }
        return false;
    ...
    public void commitSubProcessFlushState(boolean flushState) {
        try {
            mPersistentOperation.insertData(mDbParams.getSubProcessUri(),
                    new JSONObject().put(DbParams.VALUE, flushState));
        } catch (JSONException e) {
            SALog.printStackTrace(e);
        }
    }
    ...
    public void commitIdentities(String identities) {
        try {
            ...
            mPersistentOperation.insertData(mDbParams.getUserIdentities(),
                    new JSONObject().put(DbParams.VALUE, encodeIdentities));
        } catch (JSONException e) {
            SALog.printStackTrace(e);
        }
    }
    ...
    public void commitLoginIdKey(String loginIdKey) {
        try {
            mPersistentOperation.insertData(mDbParams.getLoginIdKeyUri(),
                    new JSONObject().put(DbParams.VALUE, loginIdKey));
        } catch (JSONException e) {
            SALog.printStackTrace(e);
        }
    }
    ...
    public void commitRemoteConfig(String config) {
        try {
            mPersistentOperation.insertData(mDbParams.getRemoteConfigUri(),
                    new JSONObject().put(DbParams.VALUE, config));
        } catch (Exception ex) {
            SALog.printStackTrace(ex);
        }
    }
    ...
    public String getRemoteConfigFromLocal() {
        try {
            PersistentRemoteSDKConfig persistentRemoteSDKConfig =
                    PersistentLoader.getInstance().getRemoteSDKConfig();
    ...
    public void commitPushID(String key, String pushId) {
        try {
            JSONObject jsonObject =
                    new JSONObject().put(DbParams.PUSH_ID_KEY, key).put(DbParams.PUSH_ID_VALUE,
                            pushId);
    ...
    public String getPushId(String key) {
        try {
            String[] values =
                    mPersistentOperation.queryData(mDbParams.getPushIdUri().buildUpon().appendQueryParameter(DbParams.PUSH_ID_KEY, key).build(), 1);
            if (values != null && values.length > 0) {
                return values[0];
            }
        } catch (Exception e) {
    ...
    /**
     * 根据下面两个参数查询可视化后台圈选配置的事件id
     *
     * @param screen_name
     * @param element_selector
     * @return
     */
    public String queryVisualConfigEventId(String screen_name, String element_selector,
                                           String type,String element_position) {
        try {
            return mVisualConfigsOperation.queryEventId(screen_name, element_selector, type,element_position);
        } catch (Exception e) {
            SALog.printStackTrace(e);
        }
        return null;
