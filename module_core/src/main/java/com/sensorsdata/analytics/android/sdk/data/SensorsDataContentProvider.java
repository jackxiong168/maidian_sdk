...
public class SensorsDataContentProvider extends ContentProvider {
    ...
    public int delete(Uri uri, String selection, String[] selectionArgs) {
        ...
        try {
            ...
            } else if (code == SAProviderHelper.URI_CODE.VISUAL_CONFIGS) {
                // 删除可视化后台圈选配置
                return mProviderHelper.deleteVisualConfigs(selection, selectionArgs);
            }
            //目前逻辑不处理其他 Code
        } catch (Exception e) {
            SALog.printStackTrace(e);
    ...
    public Uri insert(Uri uri, ContentValues values) {
        ...
        try {
            ...
            if (code == SAProviderHelper.URI_CODE.EVENTS) {
                ...
                // 插入可视化后台圈选配置
                return mProviderHelper.insertVisualConfig(uri, values);
            } else {
    ...
    public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs,
        ...
        try {
            int code = uriMatcher.match(uri);
            if (code == SAProviderHelper.URI_CODE.EVENTS) {
                cursor = mProviderHelper.queryByTable(DbParams.TABLE_EVENTS, projection,
                        selection, selectionArgs, sortOrder);
            } else if (code == SAProviderHelper.URI_CODE.CHANNEL_PERSISTENT) {
                cursor = mProviderHelper.queryByTable(DbParams.TABLE_CHANNEL_PERSISTENT,
                        projection, selection, selectionArgs, sortOrder);
            } else if (code == SAProviderHelper.URI_CODE.VISUAL_CONFIGS) {
                // 查询某条可视化后台圈选配置
                cursor = mProviderHelper.queryByTable(DbParams.TABLE_VISUAL_CONFIGS, projection,
                        selection, selectionArgs, sortOrder);
            } else {
