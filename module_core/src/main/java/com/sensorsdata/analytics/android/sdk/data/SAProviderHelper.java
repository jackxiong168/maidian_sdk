...
class SAProviderHelper {
    ...
    public void migratingDB(final Context context, final String packageName) {
        try {
            boolean isMigrating = AppInfoUtils.getAppInfoBundle(context).getBoolean("com" +
                    ".sensorsdata.analytics.android.EnableMigratingDB", true);
            if (!isMigrating) {
                return;
            }
                ...
                public void run() {
                    try {
                        ...
                        if (oldDatabase.exists()) {
                            final OldBDatabaseHelper oldBDatabaseHelper =
                                    new OldBDatabaseHelper(context, packageName);
                            final SQLiteDatabase database = getWritableDatabase();
                            if (database != null) {
                                final ContentValues cv = new ContentValues();
                                oldBDatabaseHelper.getAllEvents(database,
                                        new QueryEventsListener() {
                                            @Override
    ...
    public void appendUri(UriMatcher uriMatcher, String authority) {
        try {
            ...
            uriMatcher.addURI(authority, DbParams.TABLE_ACTIVITY_START_COUNT,
                    URI_CODE.ACTIVITY_START_COUNT);
            ...
            uriMatcher.addURI(authority, DbParams.TABLE_SESSION_INTERVAL_TIME,
                    URI_CODE.SESSION_INTERVAL_TIME);
            uriMatcher.addURI(authority, DbParams.PersistentName.LOGIN_ID, URI_CODE.LOGIN_ID);
            uriMatcher.addURI(authority, DbParams.TABLE_CHANNEL_PERSISTENT,
                    URI_CODE.CHANNEL_PERSISTENT);
            uriMatcher.addURI(authority, DbParams.PersistentName.SUB_PROCESS_FLUSH_DATA,
                    URI_CODE.FLUSH_DATA);
            uriMatcher.addURI(authority, DbParams.TABLE_FIRST_PROCESS_START,
                    URI_CODE.FIRST_PROCESS_START);
            uriMatcher.addURI(authority, DbParams.TABLE_DATA_DISABLE_SDK, URI_CODE.DISABLE_SDK);
            uriMatcher.addURI(authority, DbParams.PersistentName.REMOTE_CONFIG,
                    URI_CODE.REMOTE_CONFIG);
            uriMatcher.addURI(authority, DbParams.PersistentName.PERSISTENT_USER_ID,
                    URI_CODE.USER_IDENTITY_ID);
            uriMatcher.addURI(authority, DbParams.PersistentName.PERSISTENT_LOGIN_ID_KEY,
                    URI_CODE.LOGIN_ID_KEY);
            uriMatcher.addURI(authority, DbParams.PUSH_ID_KEY, URI_CODE.PUSH_ID_KEY);
            uriMatcher.addURI(authority, DbParams.TABLE_VISUAL_CONFIGS, URI_CODE.VISUAL_CONFIGS);
        } catch (Exception ex) {
     ...
     * @return
     */
    public Uri insertVisualConfig(Uri uri, ContentValues values) {
        try {
            SQLiteDatabase database = getWritableDatabase();
            if (database == null || !values.containsKey(DbParams.KEY_SCREEN_NAME)) {
                return uri;
            }
            long id = database.insert(DbParams.TABLE_VISUAL_CONFIGS, "_id", values);
     ...
     * @return
     */
    public int deleteVisualConfigs(String selection, String[] selectionArgs) {
        if (!isDbWritable) {
            return 0;
        }
        try {
            SQLiteDatabase database = getWritableDatabase();
            if (database != null) {
                return database.delete(DbParams.TABLE_VISUAL_CONFIGS, selection, selectionArgs);
            }
        } catch (SQLiteException e) {
            isDbWritable = false;
            SALog.printStackTrace(e);
    ...
    public Uri insertChannelPersistent(Uri uri, ContentValues values) {
        try {
            ...
            long d = database.insertWithOnConflict(DbParams.TABLE_CHANNEL_PERSISTENT, null,
        ...
        try {
            switch (code) {
                ...
                case URI_CODE.FLUSH_DATA:
                    mIsFlushDataState =
                            values.getAsBoolean(DbParams.PersistentName.SUB_PROCESS_FLUSH_DATA);
                    break;
    ...
    public Cursor queryByTable(String tableName, String[] projection, String selection, String[]
        ...
        try {
            ...
            if (liteDatabase != null) {
                cursor = liteDatabase.query(tableName, projection, selection, selectionArgs, null
                        , null, sortOrder);
            }
        } catch (SQLiteException e) {
            isDbWritable = false;
