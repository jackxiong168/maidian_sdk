...
public abstract class AbstractViewCrawler implements VTrack {
    ...
    private class ViewCrawlerHandler extends Handler {
        ...
        //是否把元素数据+截屏图使用Gzip压缩+Base64转成字符串 调试时可用
        ...
        private void postSnapshot(ByteArrayOutputStream out) {
            ...
            try {
                ...
                final URL url = new URL(mPostUrl + mFeatureCode);
                ...
                connection.setRequestProperty("projectId", SensorsDataAPI.getConfigOptions().getProjectId());
                connection.setRequestProperty("Content-type", "application/json");

                out2 = connection.getOutputStream();
                bout = new BufferedOutputStream(out2);
                JSONObject json = new JSONObject();
                json.put("data", out.toString());
                bout.write(json.toString().getBytes(CHARSET_UTF8));
                bout.flush();
                ...
                if (responseCode == 200) {
                    int delay = responseJson.getInt("data");
                    if (delay < 0) {
            ...
            if (rePostSnapshot) {
                mMessageThreadHandler.sendMessageDelayed(mMessageThreadHandler.obtainMessage(MESSAGE_SEND_STATE_FOR_EDITING), 5000);
            } else {
                stopUpdates(true);
            }
        }
